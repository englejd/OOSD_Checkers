...
public class BlackGamePiece extends GamePiece
{
	public BlackGamePiece (int row, int col, GameSquare[][] squares) {
		super (row, col, Player.BLACK, squares);
	}

	public BlackGamePiece (Position pos, GameSquare[][] squares) {
	...
	protected boolean validJump (Position targetPos)
	{
		...
		if (this.isKing()) {
			if ((dr == 2 || dr == -2) && ((dc == -2) || (dc == 2))) {
				if (squares[pos.r + dr][pos.c + dc].getPiece() == null) {
					if (squares[pos.r + dr/2][pos.c + dc/2].getPiece().getPlayer() == Player.RED)
						return true;
				}
			}			
		}
		
		else if ((dr == 2) && ((dc == -2) || (dc == 2))) {
			if (squares[pos.r + dr][pos.c + dc].getPiece() == null) {
				if (squares[pos.r + dr/2][pos.c + dc/2].getPiece().getPlayer() == Player.RED)
					return true;
			}
	...
	public void drawPiece (Graphics2D g2, int x, int y, int width, int height)
	{
		...
		if (isKing()) {
			g2.setColor(new Color(90,90,90)); //Grey means king
		}
		else {
